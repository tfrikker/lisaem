...
void dump_mddf(FILE *out, DC42ImageType *F) {
  ...
  if (tag[4] == 0xaa && tag[5] == 0xaa) {
    ...
    for (uint32 i = 0; i < 512; i++) {
      bootblockchecksum = ( (bootblockchecksum<<1) | ((bootblockchecksum & 0x80000000) ? 1 : 0) ) ^ sec[i] ^ i;
    }
    fprintf(out, "Bootblock Checksum: %08x\n", bootblockchecksum);
  } else { 
    fprintf(out, "Not bootable\n"); 
  }
  ...
  if (fsversion == 0x0e || fsversion == 0x0f) {
    ...
    for (mysect = 0; mysect < F->numblocks; mysect++) {  // find the extent/inode block for this file id, and fill the rest of the fields.
      ...
      if (fid > 0xefff && ((fsversion == 0x0e && TAGABSNEXT(mysect) == 0) || fsversion == 0x0f)) {
        ...
        for (int n = 1; n <= sec[0]; n++) {
          filenames[nfid][0][n - 1] = sec[n];
          filenames[nfid][0][n] = 0;
        } 
        ...
        // epoch adjust b/w Lisa and unix time
        #define ADJUST 2177452800
          
        t = (time_t)((time_t)(creationdate[nfid]) - (time_t)(ADJUST)); 
        ts = localtime(&t);
        strftime(buf1, sizeof(buf1), "%Y.%m.%d-%H:%M", ts);
        t = (time_t)((time_t)(modificationdate[nfid]) - (time_t)(ADJUST));
        ts = localtime(&t);
        strftime(buf2, sizeof(buf2), "%Y.%m.%d-%H:%M", ts);
        t = (time_t)((time_t)(accesstime[nfid]) - (time_t)(ADJUST));
        ts = localtime(&t);
        strftime(buf3, sizeof(buf3), "%Y.%m.%d-%H:%M", ts);
                                  
        filenamecleanup(filenames[nfid][0], filenames[nfid][1]); // clean it up, now output name is set
        ...
        if (serialized[nfid]) {
          snprintf(temp, 1023, "%04x    %-32s     %s %s %s  %10ld %10ld  %s %s %s branded:%08x\n",
            ...
            (passworded[nfid] ? "PASSWD" : ""), get_fileflags(fileflags[nfid]),
            get_bozo(bozo[nfid]), serialized[nfid]);
        } else {
        ...
        if (sec[0x182]) {
          int len = sec[0x182];
          for (int x = 0; x < len; x++) {
            filenames[nfid][3][x] = sec[0x183 + x];
          }
            ...
            if ((nfid == fileid) && (fileid > 4 && fileid < 0x8000) && !proccessed[fileid]) { // check filename for match, then get the rest of the fields.
              ...
              if (serialized[fileid]) {
                snprintf(temp, 1023, "%04x    %-32s     %s %s %s  %10ld %10ld  %s %s %s branded:%08x\n",
                  ...
                  (passworded[fileid] ? "PASSWD" : ""), get_fileflags(fileflags[fileid]),
                  get_bozo(bozo[fileid]), serialized[fileid]);
              } else {
                snprintf(temp, 1023, "%04x    %-32s     %s %s %s  %10ld %10ld  %s %s %s\n",
                  fileid, filenames[fileid][0], buf1, buf2, buf3, (long)size1[fileid], (long)size2[fileid],
                  (passworded[fileid] ? "PASSWD" : ""), get_fileflags(fileflags[fileid]),
...
void getcommand(void) {
  ...
  if (line[0] == '+' && len == 1) {
    command = SECTOR_NXT; 
    last_direction = SECTOR_NXT; 
    return;
  } // next sector

  if (line[0] == '-' && len == 1) {
    command = SECTOR_PRV; 
    last_direction = SECTOR_PRV; 
    return;
  } // previous sector
  ...
  if (line[0] == '+' || line[0] == '-') {
    ...
    sector += l; 
    command = DISPLAY_CMD; 
    last_direction = (line[0] == '-') ? SECTOR_PRV : SECTOR_NXT; 
    return;
  } // delta jump (len>0 i.e. +10 -5)
  if (line[0] == 'q' && len < 3) {
    command == QUITCOMMAND; 
    return;
  } // shortcut for quit
...
void printsectheader(FILE *out, DC42ImageType *F, uint32 sector) {
  ...
  if (havetags) {
    if (tagsaresorted) { // when tags are sorted, allocation bitmap is extracted.
      fprintf(out, " %s Block ", ((allocated[sector] & 8) ? "Used" : "Free"));
    }
